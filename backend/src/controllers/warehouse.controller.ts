...
import WarehouseModel, { warehouseDAO } from "./../models/warehouse.model";
import { IPaginationQuery, IReqUser } from "../utils/interfaces";

export default {
  async create(req: IReqUser, res: Response) {
    try {
      await warehouseDAO.validate(req.body);
      const result = await WarehouseModel.create(req.body);
  ...
  async findAll(req: IReqUser, res: Response) {
    ...
    try {
      ...
      const result = await WarehouseModel.find(query)
        .limit(limit)
        .skip((page - 1) * limit)
        .sort({ createdAt: -1 })
        .exec();
  ...
  async update(req: IReqUser, res: Response) {
    try {
      const { id } = req.params;
      const result = await WarehouseModel.findByIdAndUpdate(id, req.body, {
        new: true,
      });
